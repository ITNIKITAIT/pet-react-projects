...
import { getStatistic } from '../funcs/getStatistic';
import useLocalStorage from '../hooks/UseLocalStorage';

const WriteSection = ({ setPrintedWords, printedWords }) => {
    const [word, setWord] = useState('');
    const { time, start, stop } = useTimer(60);
    const [value, setValue] = useLocalStorage('wpm', 0);

    const [statisticIsVisible, setStatisticIsVisible] = useState(false);
    useEffect(() => {
        time === 0 && setStatisticIsVisible(true);
    }, [time]);
    ...
    const printWord = (e) => {
        ...
        if (newWord === ' ') return;
        if (newWord !== ' ' && printedWords.length === 0 && !word[0]) {
            start();
        }
        if (newWord.includes(' ')) {
            setWord('');
            setPrintedWords([...printedWords, newWord.trim()]);
        } else {
            setWord(newWord);
        }
    };
    ...
    return (
        <div className="main__writeSection">
            <div className="flex gap-5 items-center">
                ...
                <button
                    onClick={reload}
                    className="bg-violet-500 rounded-lg p-1">
            ...
            <div className="w-full h-5 rounded-2xl bg-transparent mt-5 border-[1px] border-slate-400 overflow-hidden">
                <div
                    style={{ width: `${(time / 60) * 100}%` }}
                    className={`bg-gradient-to-r from-violet-400 to-violet-600 h-full transition-all ease-linear duration-1000`}></div>
            </div>
            {statisticIsVisible && (
                <Statistic
                    {...result.current}
